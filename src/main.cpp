...
void QueryImage(string imageName, int Nsim)
{
   ...
   out<<"Query image: "<<imageName<<endl;
    ...
    // if(similarityVals.mtch.size() != imageHistsRE.size())
    // {
    //     cout<<"\t ERROR: Expected size of Query image for comparison with database is wrong "<<endl;

    // }
    for(int i=0; i<imageHistsRE.size(); i++)
    {
        double distance_query_to_database_i = CompareHistograms(imageHistRE, imageHistsRE[i]);
        // similarityVals.mtch[i].first = distance_query_to_database_i;
        // similarityVals.mtch[i].second = i;
...
void TestingPart(int Nsim)
{
    ...
    std::vector <matching_entry> matches(NUM_QUERY_ENTRIES);


    for(int i=0; i<imageNamesTest.size(); i++)
    {
        // cout<<"Processing image: "<<imageNamesTest[i]<<endl;
        // matches[i].mtch.resize(NUM_DB_ENTRIES);

        QueryImage(imageNamesTest[i], Nsim);
        // std::sort(matches[i].mtch.begin(),  matches[i].mtch.end());

        cout<<"Similarity values were computed"<<endl;
    }

    // std::ofstream s("/home/olga/workspace/QT/BagOfWords/results/testsave.test" );
    // boost::archive::binary_oarchive oa(s);
    // oa <<  matches;
    // std::cout << "Saved entries # " <<matches.size()<< std::endl;
    // s.close();
    // ofstream out("/home/olga/workspace/QT/BagOfWords/results/testSave.txt");
    // for(int i=0; i<matches.size(); i++)
    // {
    //     for(int j=0; j<matches[i].mtch.size(); j++)
    //         out<< matches[i].mtch[j].first<<"/"<<matches[i].mtch[j].second<<" ";
    //     out<<endl;
    // }
    // out.close();
...
vector<string> load_pxgps_log(string filename)
{
    //~ terribly crude function
    ifstream ifs(filename.c_str());
    if(!ifs)
        cerr<<"File: "<<filename<<" was not found"<<endl;
    string line;

    vector <pix_entry> entries;
    ...
    while( std::getline( ifs, line ) )
    {
        ...
        if(tokens.size() == 3)
        {
            if(tokens[0].c_str()[0] != '#')
            {
                pix_entry px;
                px.fname = tokens[0];
                imgNames.push_back(tokens[0]);
                entries.push_back(px);
